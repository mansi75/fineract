...
@Slf4j
class OrganisationCrossFeatureBoundaryTest {

    private static final String BASE = "org.apache.fineract";
    private static final String ORGANISATION_PACKAGE = "org.apache.fineract.organisation";
    ...
    private static ApplicationModule organisationModule() {
        return modules().stream() //
                .filter(module -> module.getBasePackage().getName().equals(ORGANISATION_PACKAGE)) //
                .findFirst() //
                .orElseThrow(() -> new IllegalStateException("Organisation module not found in the model"));
    }
    ...
    @Test
    @EnabledIfSystemProperty(named = "organisation.boundary.report", matches = "true")
    void printOrganisationCrossFeatureDependencyReport() {
        ...
        organisationModule().getDirectDependencies(modules()).stream().forEach((ApplicationModuleDependency dependency) -> {
        ...
        log.info("==== Organisation cross-feature dependency report ====");
    ...
    @Test
    void organisationMustNotImportOtherFeatureModules() {
        ...
        assertThat(featureDependencies) //
                .as("Organisation may depend only on fineract-core and fineract-command. Types referenced from "
