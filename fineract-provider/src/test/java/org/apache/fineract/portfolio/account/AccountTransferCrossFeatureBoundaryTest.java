...
@Slf4j
class AccountTransferCrossFeatureBoundaryTest {

    private static final String BASE = "org.apache.fineract";
    private static final String ACCOUNT_TRANSFER_PACKAGE = "org.apache.fineract.portfolio.account";
    ...
    private static ApplicationModule accountTransferModule() {
        return modules().stream() //
                .filter(module -> module.getBasePackage().getName().equals(ACCOUNT_TRANSFER_PACKAGE)) //
                .findFirst() //
                .orElseThrow(() -> new IllegalStateException("Account transfer module not found in the model"));
    }
    ...
    private static Set<String> crossFeatureDependencies() {
        Set<String> featureDependencies = new TreeSet<>();
        accountTransferModule().getDirectDependencies(modules()).stream().forEach((ApplicationModuleDependency dependency) -> {
    ...
    @Test
    @EnabledIfSystemProperty(named = "accounttransfer.boundary.report", matches = "true")
    void printAccountTransferCrossFeatureDependencyReport() {
        ...
        log.info("==== Account transfer cross-feature dependency report ====");
    ...
    @Test
    void accountTransferMustNotImportOtherFeatureModules() {
        Set<String> featureDependencies = crossFeatureDependencies();

        assertThat(featureDependencies) //
                .as("Account transfer may depend only on fineract-core and fineract-command. Types referenced from "
